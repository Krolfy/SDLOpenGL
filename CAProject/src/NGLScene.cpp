...
void NGLScene::timerEvent(QTimerEvent *_event)
{
  ...
  int BoidCount = MainFlock.GetBoidCount();
  float BoidsPos[BoidCount*3];

  m_data.resize(BoidCount);

  MainFlock.moveBoids(0.05,0.05,0.1,2,0.2);



  MainFlock.BoidsPositions(&BoidsPos[0]);

  int j = 0;
  for (int i = 0; i < BoidCount; i++)
  {

    m_data[i].m_x = BoidsPos[j];
    m_data[i].m_y = BoidsPos[j+1];
    m_data[i].m_z = BoidsPos[j+2];
    std::cout<<"pos "<< m_data[i]<<"\n";
    j+=3;
